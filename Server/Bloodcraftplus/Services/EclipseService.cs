...
internal class EclipseService
{
    ...
    /// </summary>
    /// <param name="lastSync">The dictionary tracking last sync times per user.</param>
    /// <param name="steamId">The user identifier.</param>
    /// <param name="interval">The minimum interval between syncs.</param>
    /// <returns>True when the payload should be sent.</returns>
    static bool ShouldSync(ConcurrentDictionary<ulong, DateTime> lastSync, ulong steamId, TimeSpan interval)
    {
        DateTime now = DateTime.UtcNow;

        if (!lastSync.TryGetValue(steamId, out DateTime previous) || now - previous >= interval)
        {
            lastSync[steamId] = now;
            return true;
        }
