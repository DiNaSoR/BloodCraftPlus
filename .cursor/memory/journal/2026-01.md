...
- [Docs][Config] Note startup-cached options require restart
  - Why: Prevent "toggle doesn't stop" reports when config changes are made at runtime.
  - Key files:
    - `Docs/src/content/reference/config.mdx`
    - `Docs/src/content/reference/changelog.mdx`
  - Verification:
    - `npm run build` (Docs) — PASS
  - Regression checks run:
    - Build — PASS (Docs build)
    - Lint/format — NOT RUN (docs change)
    - Unit tests — NOT RUN (not available)
    - App/service start — NOT RUN (docs-only change)
...
- [Tools][DesignMock] Migrated DesignMock from static HTML/CSS/JS to Vite + React + TypeScript
  - Why: Enable modular component development, hot reload, TypeScript type safety, and better maintainability for the UI design mock tool.
  - Key files:
    - `Tools/DesignMock/package.json` (new: Vite/React project config)
    - `Tools/DesignMock/vite.config.ts` (new: Vite config with base './')
    - `Tools/DesignMock/tsconfig.json` (new: TypeScript config)
    - `Tools/DesignMock/src/App.tsx` (main app with tab state)
    - `Tools/DesignMock/src/features/` (Equipment + Bloodcraft panels and tabs)
    - `Tools/DesignMock/src/shared/components/` (reusable Card, Select, Divider, etc.)
    - `Tools/DesignMock/src/shared/types/` (TypeScript types for tabs/modes)
    - `Tools/DesignMock/src/mock-data/` (sample data for UI)
    - `Tools/DesignMock/styles/` (relocated CSS with updated asset paths)
    - `Tools/DesignMock/public/assets/` (game sprites served statically)
    - `Tools/DesignMock/legacy/` (archived original HTML/JS/build.ps1)
    - `Docs/src/content/tools/design-mock.mdx` (updated docs for new workflow)
  - Changes:
    - New Vite + React + TypeScript project scaffold
    - Modular React components for each tab/panel/mode
    - React state replaces `design-mock.js` DOM toggles
    - CSS files moved to `styles/` folder, asset URLs updated to `/assets/...`
    - Assets moved to `public/assets/` for Vite static serving
    - Legacy files archived to `legacy/` folder with README
    - Documentation updated with new dev workflow (npm run dev/build/preview)
  - Component architecture:
    - `App` → `EquipmentPanel` | `BloodcraftPanel`
    - `BloodcraftPanel` → `PrestigeTab` | `ExoformTab` | `StatBonusesTab` | `ProgressionTab` | `FamiliarsTab`
    - `ProgressionTab` → `ProfessionsMode` | `ClassMode`
    - `FamiliarsTab` → `ManageMode` | `BattlesMode` | `TalentsMode`
  - Verification:
    - `npm install` — PASS
    - `npx tsc -b` — PASS (after fixing Card style prop + unused import)
    - `npm run build` — PASS (dist/ created with index.html + assets/)
  - Regression checks run:
    - TypeScript build — PASS
    - Vite production build — PASS
  - Notes:
    - Some asset resolution warnings during build are expected (static assets in public/)
    - Use `npm run preview` to test production build locally

## 2026-01-17

- [Tools][DesignMock] Cleaned up unused DesignMock artifacts
  - Why: Remove old build outputs and logs after migration to keep the tool directory lean.
  - Key files:
    - `Tools/DesignMock/EQUIPMENT-TAB-HIERARCHY.log` (removed)
    - `Tools/DesignMock/tsconfig.node.tsbuildinfo` (removed)
    - `Tools/DesignMock/tsconfig.tsbuildinfo` (removed)
    - `Tools/DesignMock/vite.config.d.ts` (removed)
  - Verification:
    - Not run (file cleanup only)
  - Regression checks run:
    - Not run (file cleanup only)
...
- [Docs][Process] Added a plain Markdown changelog for sharing externally
  - Why: Provide a short, human-readable changelog format that’s easy to audit in Discord without MDX/components; intentionally excludes WorldBoss work-in-progress.
  - Key files:
    - `CHANGELOG2.md` (new)
  - Verification:
...
- [Docs][Process] Converted `CHANGELOG2.md` to plain text (no Markdown)
  - Why: Some external reviewers prefer a strict plain-text changelog with zero Markdown formatting.
  - Key files:
    - `CHANGELOG2.md`
  - Verification:
    - Not run (documentation only)
...
- [Docs] Aligned Gear Level wording across changelogs + READMEs
  - Why: Remove outdated “Gear Level = Level + Expertise” phrasing and match current behavior (Equipment tab Gear Level reflects Exp level; armor hidden; expertise/class shown separately in EclipsePlus).
  - Key files:
    - `CHANGELOG.md`
    - `CHANGELOG2.md`
    - `README.md`
    ...
    - `Docs/src/content/reference/changelog.mdx`
  - Verification:
    - Searched for old phrasing — PASS
  - Regression checks run:
...
- [Docs] Reworked `CHANGELOG2.md` to separate client vs server logs
  - Why: Make audit/review easier by keeping EclipsePlus and Bloodcraftplus entries in separate sections (instead of mixing both per date).
  - Key files:
    - `CHANGELOG2.md`
  - Notes:
    - Updated Familiar Talent System wording to match implementation (server persistence, point formula, keystone costs, and talent sync via `TALENTS:` response).
  - Verification:
    - Manual review — PASS
  - Regression checks run:
    - Not run (documentation-only changes)
...
- [Docs] Redesigned docs changelog page to be full-width and plain Markdown
  - Why: Replace accordion/MDX component layout with a straightforward changelog format (bold labels + bullet lists) that reads like a traditional changelog and uses the full content width.
  - Key files:
    - `Docs/src/content/reference/changelog.mdx`
    - `Docs/src/components/Layout.tsx`
    - `Docs/src/components/Layout.css`
  - Verification:
    - Not run (docs styling/content change only)
  - Regression checks run:
...
- [Docs] Added per-plugin header styling on the docs changelog page
  - Why: Improve scanability by giving EclipsePlus/Bloodcraftplus/VDebug section headers distinct background colors inside the full-width codebox layout.
  - Key files:
    - `Docs/src/content/reference/changelog.mdx`
    - `Docs/src/styles/globals.css`
  - Verification:
...
- [Docs] Cleaned up docs changelog titles to avoid cross-plugin wording
  - Why: Don’t mix plugin names/features in another plugin’s changelog headers (e.g., no “VDebug” in Bloodcraft entries).
  - Key files:
    - `Docs/src/content/reference/changelog.mdx`
  - Verification:
    - Manual review — PASS
...
- [Docs] Disabled heading hyperlink behavior on changelog page
  - Why: Date/version headings (e.g., “January 15, 2026 — v1.12.20 …”) should not be clickable links on the changelog page.
  - Key files:
    - `Docs/src/components/mdx-components.tsx`
  - Verification:
    - Not run (docs behavior/style change only)
  - Regression checks run:
    - Not run (docs-only changes)
...
- [Docs] Reformatted changelog entries: version-first + small date subline
  - Why: Make the version the primary identifier (easier scanning), and keep the date as a secondary line (non-link).
  - Key files:
    - `Docs/src/content/reference/changelog.mdx`
    - `Docs/src/styles/globals.css`
  - Verification:
    - Quick grep for old “January … — vX (Title)” pattern — PASS
  - Regression checks run:
    - Not run (docs-only changes)
...
- [Docs] Modernized changelog entry headers (badge + title row)
  - Why: Keep the terminal/changelog vibe while improving scanability (version pill + title label + date subline).
  - Key files:
    - `Docs/src/content/reference/changelog.mdx`
    - `Docs/src/styles/globals.css`
  - Verification:
...
- [Docs] Added minimal underline styling to changelog entry titles
  - Why: Improve readability/scanability while keeping the changelog/terminal vibe; underline color follows each plugin theme.
  - Key files:
    - `Docs/src/styles/globals.css`
  - Verification:
    - Not run (docs styling/content change only)
...
- [Docs] Bundled a terminal-style font for the changelog codebox
  - Why: Ensure the changelog page uses a consistent terminal-like monospace font on any machine (not dependent on local OS fonts).
  - Key files:
    - `Docs/package.json` (added `@fontsource/jetbrains-mono`)
    - `Docs/src/main.tsx` (imports font weights)
    - `Docs/src/styles/globals.css` (disable ligatures for terminal feel)
  - Verification:
    - `npm install @fontsource/jetbrains-mono` — PASS
  - Regression checks run:
...
- [Docs] Expanded Familiar Talent System changelog details (stat explanations)
  - Why: Make the changelog self-explanatory by describing what each talent stat bonus does (including crit stats).
  - Key files:
    - `Docs/src/content/reference/changelog.mdx`
  - Verification:
    - Not run (docs content change only)
  - Regression checks run:
    - Not run (docs-only change)
...
- [Veil] Recreate HUD canvas when lost
  - Why: Veil’s persistent canvas could be destroyed during scene changes, leaving `CanvasManager` in an "initialized but not ready" state (`Canvas Ready: False`, `HudRoot: NULL`) and making the overlay invisible even when toggled.
  - Key files:
    - `Client/Veil/Core/CanvasManager.cs`
    - `Client/Veil/UI/Components/UIComponentBase.cs`
    - `Client/Veil/Plugin.cs`
    - `Client/Veil/Veil.csproj`
    - `Docs/src/content/getting-started/troubleshooting.mdx`
    - `Docs/src/content/reference/changelog.mdx`
    - `Docs/src/styles/globals.css`
  - Verification:
    - Not run (no .NET SDK available in this environment; requires in-game validation)
  - Regression checks run:
    - Build succeeds — NOT RUN (no .NET SDK)
    - UI renders + F1 toggle after joining — NOT RUN (requires V Rising client)

  - Addendum:
    - Correction: dotnet SDK was available, but not on PATH in earlier checks.
    - Build: `C:\Users\Asus\.dotnet8\dotnet.exe build -c Release` — PASS
      - `Tools/VDebug/VDebug.csproj` — PASS
      - `Client/Veil/Veil.csproj` — PASS (2 warnings: unused `_initialized` fields)
