...
// Progression - Switch between Professions and Class (within Progression tab)
document.querySelectorAll("#tab-progression .familiar-mode-tab").forEach((btn) => {
  btn.addEventListener("click", () => {
    document.querySelectorAll("#tab-progression .familiar-mode-tab").forEach((tab) => tab.classList.remove("is-active"));
    btn.classList.add("is-active");

    const mode = btn.dataset.progMode;
    document.querySelectorAll("#tab-progression .familiar-mode-content").forEach((content) => {
      content.classList.toggle("is-active", content.id === "progression-" + mode + "-content");
    });
  });
});
...
// Class Row click to toggle active (visual only for demo)
document.querySelectorAll(".class-row").forEach((row) => {
  row.addEventListener("click", () => {
    document.querySelectorAll(".class-row").forEach((r) => {
      r.classList.remove("is-active");
      const status = r.querySelector(".class-row-status");
      if (status) status.remove();
    });
    row.classList.add("is-active");
    if (!row.querySelector(".class-row-status")) {
      const status = document.createElement("span");
      status.className = "class-row-status";
...
// Class Spell Row click to toggle active (visual only for demo)
document.querySelectorAll(".class-spell-row").forEach((row) => {
  if (!row.disabled) {
    row.addEventListener("click", () => {
      document.querySelectorAll(".class-spell-row").forEach((r) => r.classList.remove("is-active"));
      row.classList.add("is-active");
    });
  }
});
