...
Common issues and their solutions when using BloodCraft.
...
**Solutions:**
1. Make sure you're connected to a server running **BloodCraft**
2. Check that `BloodUI.dll` is in the correct plugins folder
...
**Solutions:**
1. Verify `BloodCraft.dll` is in the server's plugins folder
...
**Symptoms:** Server crashes or hangs on startup with BloodCraft.
...
**Solutions:**
1. Run the server at least once with BloodCraft installed
...
**Solutions:**
1. Character data is stored per-SteamID in the BloodCraft data folder
...
**Solutions:**
1. Reduce `BloodUId` sync frequency (set to `false` for slower 2.5s updates)
...
2. **Search existing issues** — [GitHub Issues](https://github.com/DiNaSoR/BloodCraft/issues)
