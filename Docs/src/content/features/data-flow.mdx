...
This page explains how BloodCraft synchronizes data between server and client.

## Overview

BloodCraft uses a **chat-based protocol** to sync progression data:
...
│                          SERVER                                  │
│  ┌───────────────┐    ┌──────────────┐    ┌─────────────────┐  │
│  │ Game Events   │───▶│ Progression  │───▶│ BloodUIService  │  │
...
Controlled by `BloodUId` config:
- `true` — Fast sync (0.1s interval)
- `false` — Slow sync (2.5s interval)

### Data Encoding

`BloodUIService` packages progression data:
...
- Identifies BloodCraft data packets
...
| File | Purpose |
|------|---------|
| `Services/BloodUIService.cs` | Sync orchestration |
| `Interfaces/BloodUIInterface.cs` | Data packaging |
...
1. Check server has `BloodUId` features enabled
...
1. Set `BloodUId = false` for slower sync
